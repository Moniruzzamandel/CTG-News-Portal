...
<div class="breadcrumbs">
    ...
    @permission(['Post Add','All'])              
        <a href="{{ url('/back/post/create') }}" type="button" class="btn btn-outline-success btn-sm pull-right"><i class="fa fa-plus"></i>&nbsp; Create Post</a>           
    @endpermission
                      </div>
                      <div class="card-body">
        ...
        @endif 
                                    ...
                                    <td style="width: 5%"> {{ $row->category->name }}</td>            
        ...
        {{ Form::close() }}            
                                    ...
                                    <td style="width: 5%"> 
            {{ Form::open(['method'=>'PUT','url'=>['/back/post/hot_news/'.$row->id],'style'=>'display:inline' ]) }}
                @if($row->hot_news === 1)
                    {{ Form::submit('No',['class'=>'btn btn-outline-danger btn-sm']) }}
                    @else
                    {{ Form::submit('Yes',['class'=>'btn btn-outline-success btn-sm']) }}
                @endif
            {{ Form::close() }}           
                                    </td>
                                    <td style="width: 30%">
        
        <a href="{{ url('/back/comment/'.$row->id) }} " class="btn btn-outline-info btn-sm">Comment</a>
        <a href="{{ url('/back/post/edit/'.$row->id) }} " class="btn btn-outline-primary btn-sm">Edit</a>
        @permission(['Post Add','All']) 
